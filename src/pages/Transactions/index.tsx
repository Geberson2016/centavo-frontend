...
export function Transactions() {
  ...
  const { register, handleSubmit, watch, resetField } = useForm<TransactionFormData>({
    defaultValues: {
  ...
  return (
      <div className="max-w-6xl mx-auto">
        <header className="mb-10">
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Nova Transação</h1>
        </header>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div>
          ...
          <div className="flex gap-4">
            {['DESPESA', 'RECEITA'].map((t) => (
              <label key={t} className={`flex-1 p-3 rounded-xl border-2 cursor-pointer transition-all text-center font-bold ${selectedType === t ? 'border-indigo-600 bg-indigo-50 text-indigo-600' : 'border-slate-100 text-slate-400'}`}>
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-2">Descrição (Obrigatório)</label>
          <input 
        ...
        <div>
          ...
          <select 
            {...register('accountId', { required: true, valueAsNumber: true })}
        ...
        <button 
          disabled={isPending}
          type="submit"
          className="w-full bg-slate-900 text-white p-4 rounded-xl font-black hover:bg-slate-800 transition-all disabled:opacity-50"
        >
